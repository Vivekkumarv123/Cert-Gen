...
const CertificatePreview = () => {
  ...
  // ✅ Function to fix unsupported "oklch" color issues by converting them to RGB
  const fixUnsupportedColors = () => {
    document.querySelectorAll("*").forEach((el) => {
      const computedColor = window.getComputedStyle(el).color;
      if (computedColor.includes("oklch")) {
        try {
          // Extract the "oklch" value and convert to RGB using a library or custom function
          const rgbColor = convertOklchToRgb(computedColor);
  ...
  // ✅ Function to convert 'oklch' color to 'rgb' (can use a library for actual color conversion)
  const convertOklchToRgb = (oklchColor) => {
    // Placeholder for actual conversion logic
    // If you have access to a library that handles oklch-to-rgb conversion, integrate it here.
    return "rgb(0, 0, 0)"; // Example fallback to black
  };
